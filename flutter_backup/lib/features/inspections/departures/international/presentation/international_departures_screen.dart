...
class InternationalDeparturesScreen extends ConsumerStatefulWidget {
  final Inspection inspection;

  const InternationalDeparturesScreen({super.key, required this.inspection});

  @override
  InternationalDeparturesScreenState createState() =>
      InternationalDeparturesScreenState();
}

class InternationalDeparturesScreenState
    extends ConsumerState<InternationalDeparturesScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      InternationalDepartures? departure =
          await service.loadInternationalDepartureForInspection(inspectionId!);
      print(
      ...
      if (departure == null) {
        ...
        departure = InternationalDepartures.fromJson({
      ...
      print(
          "--------------- PANTALLA INICIAL -----------  ${departure.id} ${departure.inspectionDetailId}");
      ...
      MigrationData migration = MigrationData(
          hour: departure.migration.hour,
          areaInternational: departure.migration.areaInternational,
          areaNational: departure.migration.areaNational,
          timeInternational: departure.migration.timeInternational,
          timeNational: departure.migration.timeNational);
      migrationKey.currentState?.setData(migration);
  ...
  Future<void> _guardarDatos() async {
    try {
      ...
      final currentInspection = ref.read(inspectionDetailsProvider);
      ...
      final Map<String, dynamic> dataCombined = {
        ...
        //'id': arrivalId,
        'id': currentInspection?.id ?? 0,
        'synced': 0,
      };
      print("---------SAVING--------- DEPARTURE: ${currentInspection?.id}");
      final departure = InternationalDepartures.fromJson(dataCombined);
      final service = ref.read(InternationaldeparturesServiceProvider);
      print("SALIDA TO JSON");
      debugPrint("${departure.toJson()}");
      await service.save(departure);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Salida internacional guardada')),
        );
      }
    } catch (e, st) {
      debugPrint('❌ Error guardando salida internacional: $e\n$st');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    final parentKey = ValueKey(
      'international-departure-${widget.inspection.id}',
    );
    return Scaffold(
      appBar: AppBar(
        key: parentKey,
        ...
        elevation: 0,
        title: const Text('Inspección Salida Internacional',
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: TabBar(
                      ...
                      tabs: [
                        ...
                        Tab(text: 'Migración', key: _tabKeys[6]),
                        Tab(text: 'Salas de Pre-Embarque', key: _tabKeys[7]),
                        Tab(text: 'Observaciones', key: _tabKeys[8]),
                      ],
                    ),
                  ),
                ),
